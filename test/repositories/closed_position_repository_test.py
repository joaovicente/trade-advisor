...
def test_closed_position_repository_filesystem():
    ...
    assert positions[0].commission == 1
    ...
    assert positions[1].commission == 0.5

    
def test_closed_position_repository_filesystem_fail_invalid_currency():
...
def test_closed_position_repository_s3():
    ...
    path = f"{s3_bucket}/users/utest/closed_positions.csv"
    repo = ClosedPositionRepository(path=path)
    positions = repo.get_all()
