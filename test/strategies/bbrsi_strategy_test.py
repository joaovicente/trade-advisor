...
def test_buy_above_bollinger_bottom():
    ...
    date = "2023-08-18"
    ticker = "AMD"
    actions = StockComputeService(ticker, date, strategy=BbRsiStrategy).trades_today()
...
def test_sell_below_bollinger_bottom():
    ...
    date = "2023-09-21"
    actions = StockComputeService(ticker, date, open_positions=open_positions, strategy=BbRsiStrategy).trades_today()
    assert len(actions) == 1
    assert actions[0].date == parse_date(date)
